...
async function callAI(config, { system, messages, maxTokens = 4096, model }) {
  ...
  if (config.format === 'anthropic') {
    ...
    const r = await fetch(`${config.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({ model: useModel, messages: oaiMessages, max_tokens: maxTokens, thinking: { type: 'disabled' }, chat_template_kwargs: { enable_thinking: false } }),
    });
...
async function callAIStream(config, { system, messages, maxTokens = 16000, model }, onDelta) {
  ...
  if (config.format === 'anthropic') {
    ...
    const r = await fetch(`${config.baseUrl}/chat/completions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${config.apiKey}` },
      body: JSON.stringify({ model: useModel, messages: oaiMessages, max_tokens: maxTokens, stream: true, thinking: { type: 'disabled' }, chat_template_kwargs: { enable_thinking: false } }),
    });
...
app.post('/api/theme-gen', async (req, res) => {
  ...
  try {
    ...
    send({ type: 'done', theme: parsed, parseError });
    res.end();
  } catch (err) {
...
const PROMPT_GEN_MODEL = process.env.PROMPT_GEN_MODEL || MODEL;
...
const PALETTE_GEN_MODEL = process.env.PALETTE_GEN_MODEL || MODEL;
